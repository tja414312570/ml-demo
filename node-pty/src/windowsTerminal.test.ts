  ...
  return new Promise<IWindowsProcessTreeResult[]>(resolve => {
    ...
    const interval = setInterval(() => {
      psList({ all: true }).then((ps:any) => {
        ...
        openList.push(ps.filter((p:any) => p.pid === pid).map((p:any) => {
          return { name: p.name, pid: p.pid };
        })[0]);
        const list: IWindowsProcessTreeResult[] = [];
        while (openList.length) {
          const current = openList.shift()!;
          ps.filter((p:any) => p.ppid === current.pid).map((p:any) => {
            return { name: p.name, pid: p.pid };
          }).forEach((p:any) => openList.push(p));
          list.push(current);
        }
