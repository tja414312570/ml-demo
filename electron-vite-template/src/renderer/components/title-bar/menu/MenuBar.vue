<template>
  <div :class="[
    ...
    <div v-if="options.mini" ref="menuBarContent" class="mx-menu-bar-content">
      <div class="mx-menu-bar-item" @click="onItemClick(0, null)">
        <MenuBarIconMenu />
      </div>
    </div>
    <div v-else ref="menuBarContent" class="mx-menu-bar-content">
      <div v-for="(item, key) in menuItems" :key="key" :class="[
        'mx-menu-bar-item',
        item == menuActive ? 'active' : '',
      ]" @click="onItemClick(key, item as MenuItem)" @mouseenter="onItemEnter(key, item as MenuItem)">
        {{ item.label }}
      </div>
    </div>
    <slot name="suffix" />
